...
function createInkdTools() {
  ...
  const checkBalanceTool = new DynamicStructuredTool({
    ...
    description:
      "Check the agent wallet's $INKD token balance. You need at least 1 $INKD to register on-chain.",
    schema: z.object({}),
    func: async () => {
      ...
      return `Wallet ${account.address} has ${formatEther(balance)} $INKD`;
    },
  });
  ...
  const registerAgentTool = new DynamicStructuredTool({
    ...
    description:
      "Register this AI agent on the Inkd Protocol. Creates a permanent on-chain identity. Costs 1 $INKD (locked). Returns the project ID.",
    schema: z.object({
      name: z
        .string()
   ...
   */
  const pushMemoryTool = new DynamicStructuredTool({
    name: "inkd_push_memory",
    description:
      "Permanently store agent memory/state on Arweave and record it on-chain as a new version. Use this to persist knowledge, learnings, or state.",
    schema: z.object({
      projectId: z
        .string()
      ...
      memory: z
        .string()
        .describe(
          "The content to store permanently. Can be JSON, text, or any string."
        ),
      versionTag: z
        .string()
  ...
  const discoverAgentsTool = new DynamicStructuredTool({
    ...
    description:
      "Discover other AI agents registered on Inkd Protocol. Returns their names, wallet addresses, and API endpoints.",
    schema: z.object({
      limit: z
        .number()
        .optional()
    ...
    func: async ({ limit }) => {
      try {
        ...
        if (!ids.length) return "No agents registered yet.";
  ...
  const getProjectTool = new DynamicStructuredTool({
    ...
    func: async ({ projectId }) => {
      try {
        ...
        if (!p.exists) return `Project #${projectId} not found.`;
