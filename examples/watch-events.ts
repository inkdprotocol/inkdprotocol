...
import { createPublicClient, http, decodeEventLog, parseAbi, type Log, type Hex } from 'viem'
import { baseSepolia, base } from 'viem/chains'

const REGISTRY_ADDRESS = process.env['INKD_REGISTRY'] as `0x${string}` | undefined
...
function handleLog(log: Log) {
  try {
    ...
    switch (decoded.eventName) {
      case 'ProjectCreated': {
        const a = decoded.args as { projectId: bigint; owner: string; name: string; isAgent: boolean }
      ...
      case 'VersionPushed': {
        const a = decoded.args as { projectId: bigint; versionIndex: bigint; arweaveHash: string; versionTag: string }
      ...
      case 'ProjectTransferred': {
        const a = decoded.args as { projectId: bigint; from: string; to: string }
