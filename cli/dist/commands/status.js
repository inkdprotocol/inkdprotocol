...
async function cmdStatus() {
    ...
    try {
        ...
        const [versionFee, transferFee, projectCount] = await Promise.all([
            client.readContract({ address: addrs.registry, abi: abi_js_1.REGISTRY_ABI, functionName: 'versionFee' }),
            client.readContract({ address: addrs.registry, abi: abi_js_1.REGISTRY_ABI, functionName: 'transferFee' }),
            client.readContract({ address: addrs.registry, abi: abi_js_1.REGISTRY_ABI, functionName: 'projectCount' }),
        ]);
        console.log();
        (0, config_js_1.info)(`Projects:      ${config_js_1.GREEN}${projectCount.toString()}${config_js_1.RESET}`);
        (0, config_js_1.info)(`Version fee:   ${config_js_1.GREEN}${(0, viem_1.formatEther)(versionFee)} ETH${config_js_1.RESET}`);
        (0, config_js_1.info)(`Transfer fee:  ${config_js_1.GREEN}${(0, viem_1.formatEther)(transferFee)} ETH${config_js_1.RESET}`);
    }
    catch (e) {
