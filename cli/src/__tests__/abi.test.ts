...
describe("REGISTRY_ABI", () => {
  ...
  it("every entry has a name and type", () => {
    for (const entry of REGISTRY_ABI as AbiEntry[]) {
  ...
  it("all function entries have stateMutability", () => {
    const functions = (REGISTRY_ABI as AbiEntry[]).filter((e) => e.type === "function");
  ...
  for (const fnName of expectedWriteFns) {
    it(`includes write function "${fnName}"`, () => {
      const entry = (REGISTRY_ABI as AbiEntry[]).find((e) => e.name === fnName);
  ...
  it("createProject has all required input fields", () => {
    const entry = (REGISTRY_ABI as AbiEntry[]).find((e) => e.name === "createProject")!;
  ...
  it("pushVersion is payable", () => {
    const entry = (REGISTRY_ABI as AbiEntry[]).find((e) => e.name === "pushVersion")!;
  ...
  it("transferProject is payable", () => {
    const entry = (REGISTRY_ABI as AbiEntry[]).find((e) => e.name === "transferProject")!;
    expect(entry.stateMutability).toBe("payable");
  });
  ...
  it("getProject is a view function", () => {
    const entry = (REGISTRY_ABI as AbiEntry[]).find((e) => e.name === "getProject")!;
  ...
  it("contains only function-type entries (CLI ABI is functions-only)", () => {
    const nonFunctions = (REGISTRY_ABI as AbiEntry[]).filter((e) => e.type !== "function");
...
describe("TOKEN_ABI", () => {
  ...
  it("every entry has a name and type", () => {
    for (const entry of TOKEN_ABI as AbiEntry[]) {
      expect(typeof entry.name).toBe("string");
      expect(entry.name.length).toBeGreaterThan(0);
      expect(typeof entry.type).toBe("string");
    }
  ...
  it("transfer is nonpayable with to+amount inputs", () => {
    const entry = (TOKEN_ABI as AbiEntry[]).find((e) => e.name === "transfer")!;
    expect(entry.stateMutability).toBe("nonpayable");
    const inputNames = entry.inputs!.map((i) => i.name);
  ...
  it("allowance has owner+spender inputs", () => {
    const entry = (TOKEN_ABI as AbiEntry[]).find((e) => e.name === "allowance")!;
    const inputNames = entry.inputs!.map((i) => i.name);
  ...
  it("all function entries have stateMutability", () => {
    const functions = (TOKEN_ABI as AbiEntry[]).filter((e) => e.type === "function");
