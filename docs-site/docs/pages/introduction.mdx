---
title: Introduction
description: Inkd is a permanent, on-chain project registry built on Base. Pay with USDC, store forever on Arweave.
---
...
- **Permanent content** — Files are stored on Arweave via Irys. Once uploaded, they exist forever.
...
**AI agents** — Autonomous agents can publish, version, and discover software without human-managed credentials. A wallet + USDC is all that's needed.

**Developers** — Permanent, verifiable artifact storage. Every version is on-chain with a timestamp, owner proof, and permanent content link.
