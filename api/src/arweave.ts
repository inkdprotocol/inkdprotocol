...
let cachedArUsd: number | null = null
let cacheExpiry = 0
...
async function getArUsdPrice(): Promise<number> {
  if (cachedArUsd && Date.now() < cacheExpiry) return cachedArUsd
 ...
 */
async function getArweavePriceWinston(bytes: number): Promise<bigint> {
  const res = await fetch(`${ARWEAVE_GW}/price/${bytes}`, {
    signal: AbortSignal.timeout(5000),
  })
  if (!res.ok) throw new Error(`Arweave price error: ${res.status}`)
 ...
 */
function winstonToAr(winston: bigint): number {
 ...
 */
export async function getArweaveCostUsdc(bytes: number): Promise<bigint> {
 ...
 */
export function calculateCharge(arweaveCostUsdc: bigint, markupBps = 2000) {
