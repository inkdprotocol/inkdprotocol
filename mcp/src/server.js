...
// ─── Config ───────────────────────────────────────────────────────────────────
const PRIVATE_KEY = process.env['INKD_PRIVATE_KEY'];
...
async function main() {
    ...
    const server = new index_js_1.Server({ name: 'inkd', version: '0.1.0' }, { capabilities: { tools: {} } });
    // List tools
        ...
        try {
            switch (name) {
                ...
                case 'inkd_push_version': {
                    const { projectId, tag, contentHash, metadataHash } = args;
                ...
                case 'inkd_upload': {
                    const { content, contentType = 'text/plain', filename } = args;
                ...
                case 'inkd_add_collaborator': {
                    ...
                    const { manifestHash, collaboratorAddress, collaboratorPublicKey } = args;
                    ...
                    const { addRecipientToManifest } = await Promise.resolve(`${'@inkd/sdk'}`).then(s => __importStar(require(s)));
    ...
    if (address)
        console.error(`[inkd-mcp] Wallet: ${address}`);
    else
        console.error(`[inkd-mcp] Read-only mode (no INKD_PRIVATE_KEY)`);
}
main().catch(err => {
